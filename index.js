...
const productNodes = [...dom.window.document.querySelectorAll(".lb-content-item")];
...
const products = productNodes
  .map(productNode => {
    return {
      ...
      category: productNode.parentNode.parentNode.querySelector('span').textContent
    };
  })
  .sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' }));

const grouped = Object.entries(groupBy(products, product => product.name)).map(([k, vs]) => {
  return {
    name: k,
    link: vs[0].link,
